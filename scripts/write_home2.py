...
import {
  company, kpiArray, decisions, signals,
  topDecisionHome, valueAtStake, whatChanged,
} from '../data/gisData';
...
const HubCard = ({ to, title, subtitle, icon, color, bg }) => {
  ...
  return (
    <div
      className="rounded-2xl p-6 cursor-pointer transition-all hover:-translate-y-0.5 hover:shadow-md"
      style={{ background: '#ffffff', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}
      onClick={() => navigate(to)}
    >
  ...
  return (
    <div className="p-6 space-y-6" style={{ background: '#f4f6f9', minHeight: '100%' }}>
      ...
      <div className="flex items-start justify-between">
        ...
        <button
          className="text-xs font-semibold px-5 py-2.5 rounded-xl text-white transition-opacity hover:opacity-90 flex items-center gap-1.5"
      ...
      {/* ── TOP ROW: Top Decision | Value at Stake | What Changed ── */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          ...
          <div className="text-xs mb-3" style={{ color: '#64748b' }}>{td.subtitle}</div>
          <div className="flex gap-2 text-xs mb-3">
            <span className="px-2 py-0.5 rounded-full" style={{ background: '#f8fafc', color: '#64748b', border: '1px solid #e2e8f0' }}>
              Confidence: {td.confidence}
            </span>
            <span className="px-2 py-0.5 rounded-full" style={{ background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca' }}>
          ...
          <div className="flex gap-2">
            ...
            >See All ({decisions.length})</button>
          </div>
        </div>
        ...
        >
          ...
          {vas.map(v => (
            <div key={v.label} className="flex items-center justify-between py-2" style={{ borderBottom: '1px solid #f1f5f9' }}>
              <span className="text-xs" style={{ color: '#334155' }}>{v.label}</span>
              <span
                className="text-xs font-bold"
                style={{ color: v.positive ? '#16a34a' : '#dc2626' }}
              >{v.value}</span>
            </div>
          ))}
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <HubCard
            to="/app/insights"
            title="Insights Hub"
            subtitle="Signals, KPIs & recommendations"
            color="#2563eb"
            bg="#eff6ff"
          ...
          <HubCard
            to="/app/decisions"
            title="Decision Hub"
            subtitle="Review and commit decisions"
            color="#7c3aed"
            bg="#f5f3ff"
          ...
          <HubCard
            to="/app/execution"
            title="Execution Hub"
            subtitle="Track actions and milestones"
            color="#0891b2"
            bg="#ecfeff"
...
out_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'src', 'pages', 'HomePage.jsx')
